...
pg %>% count(year,type2) %>% pivot_wider(names_from=type2, values_from=n, values_fill = 0) %>% mutate(n = agg+hyb+ten)
...
# pg.sdiff <- pg %>% pivot_longer(all_of(floraltraits.pg)) %>% drop_na(value) %>% 
#   group_by(name) %>% nest() %>% 
#   mutate(model = map(data, ~lm(seeds_est ~ value * factor(year), data=.)), 
#          emt = map(model, ~ summary(emtrends(.x, ~year, var="value")))) %>% 
#   select(-data, -model) %>% unnest(emt)
