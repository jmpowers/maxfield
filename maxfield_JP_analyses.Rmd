...
(EPA_89_06_July_avg_mm <- daily_GTH161 %>% filter(month(date)==7, year(date) >= 1989, year(date) <= 2006) %>% 
...
ggplot(aes(x=mo*30.4-15.2, y=precipitation_mm, color=station)) + #place points at month midpoint
  facet_wrap(vars(yr)) +  
    geom_rect(data=waterdates %>% pivot_wider(names_from=precip_treatments, values_from=c(day, date)) %>% mutate(yr=factor(year)), 
            aes(xmin=day_started, xmax=day_ended, ymin=0, ymax=(14 / 4 / 2) * 31), inherit.aes=F, fill=water_pal["Addition"])+
...
```{r summerprecip}
treatments %>% ggplot(aes(x=snow, shape=snow, color=water, y=precip_est_mm))+geom_point()+ 
...
ggplot(emm.melt.df, aes(x=fct_relevel(trait, rev(traits)), y=trend)) +  
  ...
  labs(y="Slope of z-scored trait vs. predictor", x="Floral Trait", color="Water") + scale_x_discrete(labels = traitnames) + coord_flip() + scale_color_manual(values=water_pal) +
