...
Tally the number of each status combination for each year. Note this this is now happening after duplicates are resolved.

```{r load_census}
nrf_count <- bind_rows(
    "2018"=allowed_nrf %>% filter(year==2018) %>% left_join(cen18 %>% mutate(nrf=paste(notes, rosettes, flowering)) %>% group_by(nrf) %>% tally()),
  "2018r2"=allowed_nrf %>% filter(year==2018) %>% left_join(cen18r2 %>% mutate(nrf=paste(notes, rosettes, flowering)) %>% group_by(nrf) %>% tally()),
  "2019"=allowed_nrf %>% filter(year==2019) %>% left_join(cen19 %>% mutate(nrf=paste(notes, rosettes, flowering)) %>% group_by(nrf) %>% tally()),
  "2020"=allowed_nrf %>% filter(year==2020) %>% left_join(cen20 %>% mutate(nrf=paste(notes, rosettes, flowering)) %>% group_by(nrf) %>% tally()), .id="year_round")

nrf_count %>% select(-nrf) %>% arrange(year_round, status, desc(n)) %>% drop_na(n) %>% kable
```

Visualize a plant's status in each census

```{r status}
# megatally %>% select(where(is.numeric)) %>% 
#   relocate(total_obs,census18,census18r2,census19,census20,pheno18,seeds18,floral18,pheno19,seeds19,floral19,pheno20,seeds20,floral20,volatiles20, leaf18,leaf19,leaf20,census18r2) %>% 
#   arrange(census18>0,cut(pheno18,c(0,1,2,Inf),right=F),seeds18>0,census19>0,cut(pheno19,c(0,1,2,Inf),right=F),seeds19>0,census20>0,cut(pheno20,c(0,1,2,Inf),right=F),seeds20>0, leaf18>0,leaf19>0,leaf20>0,census18r2>0,total_obs) %>% 
#   decostand("max") %>% sqrt %>% as.matrix %>% 
#   heatmap(Colv=NA, Rowv = NA, scale="none", col=viridis(512))
...
```{r compare_years19}
sort(setdiff(cen19[cen19$notes != "tagnf",]$plantid, cen20$plantid))
```

Recorded in 2020 but not in 2019:
```{r compare_years20}
sort(setdiff(cen20[cen20$notes != "tagnf",]$plantid, cen19$plantid))
```
...
```{r transitions}
with(cen, table(`2019`=paste(status_19), `2020`=paste(status_20)))

ggplot(filter(cen, status_19 %in% status_ok, status_20 %in% status_ok), aes(x=status_19, fill=status_20)) + geom_bar(position="fill") + scale_fill_manual(values=status_pal) + labs(fill="2020 Status", y="Proportion", x="2019 Status")
```

## Survival and flowering vs. plant size
```{r surv_size}
ggplot(filter(cen, status_20 %in% status_ok), aes(x=r1_longest_19, fill=status_20)) + facet_wrap(vars(status_20), ncol=1) +
  geom_density() + scale_fill_manual(values=status_pal) + labs(x="2019 Rosette 1 Longest Leaf (mm)", fill="2020 status")
ggplot(filter(cen, status_20 %in% status_ok), aes(x=r1_leaves_19, fill=status_20)) + facet_wrap(vars(status_20), ncol=1) +
...
```{r growth}
ggplot(drop_na(cen, water_19, snow_19), aes(x=r1_longest_19, y=r1_longest_20, color=water_19, shape=snow_19)) + geom_point() + geom_abline(intercept=0, slope=1) + scale_color_manual(values=water_pal) + theme_minimal()+ labs(x="2019 Rosette 1 Longest Leaf (mm)", y="2020 Rosette 1 Longest Leaf (mm)",  color="Precipitation", shape="Snowmelt")
ggplot(drop_na(cen, water_19, snow_19), aes(x=r1_leaves_19, y=r1_leaves_20, color=water_19, shape=snow_19)) + geom_point() + geom_abline(intercept=0, slope=1 )+ scale_color_manual(values=water_pal) + theme_minimal()+ labs(x="2019 Rosette 1 # Leaves", y="2020 Rosette 1 # Leaves",  color="Precipitation", shape="Snowmelt")
```
