...
## Measures of all plants in all years
...
megatally <- list(
  ...
  reduce(full_join) %>% arrange(plotid, plant) %>% rowwise() %>% 
  mutate(total_obs = sum(c_across(3:17), na.rm=T))
...
cen18 %>% group_by(id) %>% filter(n()>1) %>% arrange(id, status, desc(date)) %>% 
  write_sheet(ss=filter(datasheets, name=="Maxfield Results"), sheet="2018duplicates")
cen18r2 %>% group_by(id) %>% filter(n()>1) %>% arrange(id, status) %>% 
  write_sheet(ss=filter(datasheets, name=="Maxfield Results"), sheet="2018r2duplicates")
...
cen18 <- cen18 %>% arrange(id, status, desc(date)) %>% distinct(id, .keep_all = TRUE)
cen18r2 <- cen18r2 %>% arrange(id, status, desc(date)) %>% distinct(id, .keep_all = TRUE)
...
### 2020
```{r display_size}
ph.plantid <- ph %>% left_join(sm.subplotyear) %>% group_by(year, plotid, plantid, snow, water) %>% summarize_at(c("open","height_cm", "VWC"), mean, na.rm=T) 
...
```{r seeds}
ggplot(sds, aes(color=water, x=fruits, y=seeds)) + geom_point() + geom_smooth(se=F) + 
  scale_color_manual(values=water_pal) + facet_wrap(vars(year), nrow=1)

seedtraits <- c("seeds_per_fruit", "seeds_est", "fruits_nonaborted",  "flowers_est", "prop_fly", "prop_aborts")
seedtraitnames <- setNames(c("Seeds per fruit", "Estimated total seeds", "Nonaborted fruits", "Estimated total flowers", "Proportion of nonaborted fruits with flies","Proportion of fruits that aborted"), seedtraits)
