...
```{r hobo}
ggplot(hobo %>% filter(month(time)%in%4:6), aes(x=time, y=temp, color=plot)) + facet_wrap(vars(year), scales="free_x", ncol=1) + 
  geom_line() + geom_hline(yintercept=melt_threshold_warm) +
  geom_segment(data=meltdates, aes(x=warm_time, y=-10, xend=melt_time, yend=-15, color=plot), size=1) +
...
ggplot(hobo %>% filter(month(time)%in%4:6), aes(x=time, y=intensity, color=plot)) + facet_wrap(vars(year), scales="free_x", ncol=1) + 
...
left_join(sm.subplotmonth, meltdates) %>% 
  ...
  ggplot(aes(x=melt_date, y=VWC, shape=year,color=water)) + 
...
ggplot(sm, aes(x=yday(date), shape=snow, linetype=snow, color=water, y = VWC)) + 
  ...
  geom_vline(data=meltdates, aes(xintercept=melt_date, linetype=snow), inherit.aes=F, color=brewer.pal(3, "Purples")[3])+
...
### Floral plasticity: water treatments and snowmelt
...
mt.mod.raw <- mt.mod.plantyr <- mt.mod.subplot <- mt.mod.subplot.test <- mt.mod.subplot.emm <- mt.mod.plot <- list()
for(tr in traits) {
  ...
  # Use model with data averaged by subplot
  ...
  # Plot the model predictions with the data average by plant 
  mt.mod.plot[[tr]] <- 
...
mt.mod.tests <- map_dfr(mt.mod.subplot.test, tidy, .id="trait")
mt.mod.tests %>% select(c("trait", "term", "p.value")) %>% mutate(p.value = ifelse(p.value < 0.05,"*","")) %>% 
  pivot_wider(names_from=trait, values_from=p.value) %>% kable
```
...
mt.sm.mod.tests %>% select(c("trait", "term", "p.value")) %>% mutate(p.value = ifelse(p.value < 0.05,"*","")) %>% 
  pivot_wider(names_from=trait, values_from=p.value) %>% kable
...
ggplot(sm, aes(x=yday(date))) + 
  ...
  geom_vline(data=meltdates, aes(xintercept=melt_date, linetype=snow), color="grey40")+
