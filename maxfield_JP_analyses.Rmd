...
meltdates %>% ggplot(aes(x=melt_offset, fill=snow)) + geom_dotplot(binwidth=1, color="white") +
...
* *snow_new*, which has plot 5 as "early" in 2019 (it melted 1 day earlier than the next normal plot)
* *water4*, which does not combine the mock rainout treatments with the controls
* *plot* as a random effect (subplots vary within this larger unit)

The models are specified as:

    lmer(trait ~ year * snow_new * water4 + (1|plot), data = mt.subplot)
...
for(tr in traits) {
  mt.mod[[tr]] <- lmer(mt.subplot[[tr]] ~ year * snow_new * water4 + (1|plot), data=mt.subplot)
  mt.mod.test[[tr]] <- anova(mt.mod[[tr]])
  mod.emm[[tr]] <- summary(emmeans(ref_grid(mt.mod[[tr]]), ~ year + snow_new * water4))
  
  mt.mod.plot[[tr]] <- 
    ggplot(mod.emm[[tr]], aes(x=water4, color=snow_new, y = emmean)) + facet_wrap(vars(year), nrow=1) +
        labs(x="Precipitation", y=traitnames[[tr]], color="Snowmelt\n(2019 plot 5=early)", shape="Year") +
        geom_point(data=mt.subplot, aes_string(x="water4", color="snow_new", y = tr), inherit.aes = F, position=position_dodge(0.7)) + 
...
The models are specified as: 
    ...
    lmer(trait ~ year * water * melt_offset + (1|plotid), data = mt.plantyr)

```{r morphology_model}
options(contrasts = c("contr.sum", "contr.poly"))
mt.mod.raw <- mt.mod.plantyr <- mt.mod.subplot <- mt.mod.plantyr.test <- mt.mod.plot <- list()
for(tr in traits) {
  ...
  mt.plantyr[[paste0("fit.",tr)]][!is.na(mt.plantyr[[tr]])]<- predict(mt.mod.plantyr[[tr]], re.form=NA)
  mt.mod.plantyr.test[[tr]] <- anova(mt.mod.plantyr[[tr]])
  
  mt.mod.plot[[tr]] <- 
    ggplot(mt.plantyr, aes_string(x="melt_offset", color="water", y = tr)) + geom_point() +
...
mt.mod.plantyr.tests <- map_dfr(mt.mod.plantyr.test, tidy, .id="trait")
mt.mod.plantyr.tests %>% select(c("trait", "term", "p.value")) %>% mutate(p.value = signif.num(p.value)) %>% 
  pivot_wider(names_from=trait, values_from=p.value) %>% 
  bind_rows(lapply(mt.mod.plantyr, function(x) ifelse(isSingular(x),"Y","N"))) %>% mutate(term = replace_na(term,"Singular?")) %>% kable
```
...
ggplot(ph20.plantid, aes(x=height_cm, y=open, color=water)) + geom_point() + geom_smooth(method="lm", se=F) + scale_color_manual(values=water_pal) +
  labs(x="Mean inflorescence height (cm)", y="Mean number of open inflorescences", color="Precipitation")
...
ggplot(ph20.plantid, aes(x=water, color=snow, y = open)) +
    labs(x="Precipitation", y="Mean number of open inflorescences", color="Snowmelt") +
    geom_boxplot() + scale_color_manual(values=snow_pal) 
ggplot(ph20.plantid, aes(x=VWC, y=open)) + geom_point() + geom_smooth()+
    labs(x="Mean VWC", y="Mean number of open inflorescences")
```
