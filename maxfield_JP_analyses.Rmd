...
summer_precip_timing = summer_precip_timing %>% group_by(year) %>% summarize(begin=min(day), end=max(last_day)),
...
### FLoral plasticity: absolute snowmelt date
...
for(tr in traits) {
  ...
  mt.mod.subplot[[tr]] <- lmer(mnps.subplot[[tr]] ~ melt_date * water  + (1|plot), data=mnps.subplot)
  mnps.subplot[[paste0("fit.",tr)]] <- NA
  mnps.subplot[[paste0("fit.",tr)]][!is.na(mnps.subplot[[tr]])]<- predict(mt.mod.subplot[[tr]], re.form=NA)
  mt.mod.subplot.test[[tr]] <- anova(mt.mod.subplot[[tr]])
  
    #Z-score the data first to get standardized slopes
  mt.mod.subplot[[tr]] <- lmer(scale(mnps.subplot[[tr]]) ~ melt_date + water + melt_date:water + (1|plot), data=mnps.subplot)
  coef.melt.precip[[tr]] <- summary(mt.mod.subplot[[tr]])$coefficients[c("melt_date"),]#,"early_precip_mm"
  emm.melt[[tr]] <- emtrends(mt.mod.subplot[[tr]],  ~ water, var = "melt_date")
  #emm.precip[[tr]] <- emtrends(mt.mod.subplot[[tr]],  ~ water, var = "early_precip_mm")
  
  mt.mod.plot[[tr]] <-
    ggplot(mnps.subplot, aes_string(x="melt_date", color="water", y = tr, shape="year")) + geom_point() +
    geom_line(data=mnps.subplot %>% drop_na(paste0("fit.",tr)) %>% mutate(year.water = paste(year, water)), aes_string(y=paste0("fit.",tr), group="year.water"), size=2)+
     scale_color_manual(values=water_pal)+
    scale_shape_discrete(labels=with(summer_precip, paste0(year, ": ", round(early_precip_mm), " mm")))+
        labs(x="Snow melt date (day of year)", y=traitnames[[tr]], color="Precipitation", shape="Year: Summer precip")
  print(mt.mod.plot[[tr]])
}
mt.mod.subplot.tests <- map_dfr(mt.mod.subplot.test, tidy, .id="trait")
mt.mod.subplot.tests %>% select(c("trait", "term", "p.value")) %>% mutate(p.value = signif.num(p.value)) %>% 
...
emm.melt.df <- bind_rows(melt_date = emm.melt %>% map(as.data.frame) %>% bind_rows(.id="trait") %>% rename(trend=melt_date.trend)
          #,early_precip_mm = emm.precip %>% map(as.data.frame) %>% bind_rows(.id="trait") %>% rename(trend=early_precip_mm.trend)
...
ggplot(emm.melt.df, aes(x=fct_relevel(trait, rev(traits)), y=trend, color=water)) +  
