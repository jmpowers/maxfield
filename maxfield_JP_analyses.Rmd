...
# Weather data
```{r weather}
ggplot(daily_CORBIL, aes(y=precip_mm, x=julian)) + geom_col(fill=brewer.pal(3, "Greens")[3]) + facet_wrap(vars(year), ncol=1) + labs(x="Day of year", y="Precipiation (mm)") + theme_minimal() + coord_cartesian(xlim=c(120,325),ylim=c(0,20))+
  geom_point(data=groundcover %>% filter(year>=2015) %>% mutate(year=factor(year)), aes(x=first_0_cm_day,y=10))+
  geom_point(data=groundcover %>% filter(year>=2016) %>% mutate(year=factor(year-1)), aes(x=first_snow_day,y=10))

inner_join(daily_CORBIL,daily_KCOMTCRE2) %>% mutate(precip_mm = ifelse(precip_mm>50, NA, precip_mm)) %>% 
  ggplot(aes(x=Precip, y=precip_mm))+geom_point(alpha=0.6) +geom_smooth(method="lm") + geom_abline(slope=1, intercept=0)+ 
  coord_fixed(xlim=c(0,50), ylim=c(0,50))+labs(x="KCOMTCRE2 daily precip (mm)",y="billy daily precip if <50 mm (mm)")

inner_join(daily_billy,daily_KCOMTCRE2) %>% 
  ggplot(aes(x=Precip, y=prcp_mm))+geom_point(alpha=0.6) +geom_smooth(method="lm") + geom_abline(slope=1, intercept=0)+ 
  coord_fixed(xlim=c(0,50), ylim=c(0,50))+labs(x="KCOMTCRE2 daily precip (mm)",y="billy daily precip (mm)")

inner_join(daily_billy,daily_CORBIL) %>% mutate(precip_mm = ifelse(precip_mm>50, NA, precip_mm)) %>%
  ggplot(aes(y=precip_mm, x=prcp_mm))+geom_point(alpha=0.6) +geom_smooth(method="lm") + geom_abline(slope=1, intercept=0)+ 
  coord_fixed(xlim=c(0,50), ylim=c(0,50))+labs(y="WRCC billy daily precip (mm)",x="GHCND billy daily precip if <50 mm (mm)")

inner_join(daily_GTH161,daily_KCOMTCRE2) %>%
  ggplot(aes(x=Precip, y=PRECIPITATION))+geom_point(alpha=0.6) +geom_smooth(method="lm") + geom_abline(slope=1, intercept=0)+ coord_fixed(xlim=c(0,50), ylim=c(0,50))+labs(x="KCOMTCRE2 daily precip (mm)",y="EPA GTH161 daily precip (mm)")

monthly_all <- daily_billy %>% left_join(daily_GTH161) %>% left_join(daily_KCOMTCRE2) %>% left_join(daily_CORBIL) %>% 
  mutate(mo = month(date, label=F), yr = year(date)) %>% 
  mutate(precip_mm = ifelse(precip_mm>50, NA, precip_mm)) %>% #for daily_CORBIL
  group_by(mo, yr) %>% summarize(across(c("PRECIPITATION","Precip","precip_mm","prcp_mm"), #throw out month total if 5+ days missing
                                        ~ ifelse(sum(is.na(.x))>5, NA, sum(.x, na.rm=T))), .groups="drop") %>% 
  rename(EPA_RsrchMdw=PRECIPITATION, ESSDIVE_GoldLink=Precip, WRCC_billy=precip_mm, NOAA_billy=prcp_mm) %>% 
  pivot_longer(EPA_RsrchMdw:NOAA_billy, names_to="station", values_to="precipitation_mm")

ggplot(monthly_all, aes(x=mo*30.4, y=precipitation_mm, color=station)) + facet_wrap(vars(yr)) +
  geom_line() +  
  ...
  labs(y="Monthly precipitation (mm)", x="Day of the year, with snowmelt and first snow dates", color="Station") + theme(legend.position="top") 

#recalculate Campbell and Wendlandt 2013 July average, 1989 - 2006 EPA station
daily_GTH161 %>% filter(month(date)==7, year(date) >= 1989, year(date) <= 2006) %>% 
  group_by(year(date)) %>% summarize(PRECIPITATION = sum(PRECIPITATION)) %>% pull(PRECIPITATION) %>% mean

(14 / 4 / 2) * 31 #14 L applied to 4 m2 every 2 days = 1.75 mm daily precip * 31 days in July

monthly_all %>% filter(mo %in% 5:7, yr %in% as.character(2018:2020)) %>% pivot_wider(names_from=station, values_from=precipitation_mm) %>% arrange(yr)
...
ggplot(sm, aes(x=yday(date))) + 
  ...
  geom_col(data=daily_precip %>% filter(year!="2017"), aes(x=julian, y=precip_mm), 
           fill=brewer.pal(3, "Greens")[3]) +
  geom_line(data=daily_temp %>% filter(year!="2017"), aes(x=julian, y=av_temp_2m_C), 
  ...
  geom_segment(data=water_dates %>% pivot_wider(names_from=precip_treatments, values_from=c(day,date)),
               aes(x=day_started, xend=day_ended, y=20, yend=20)) +
  geom_point(data=water_dates, aes(x=day,y=20)) +
...
Summer precipitation estimates
```{r summerprecip}
summer_precip %>% ggplot(aes(x=snow, shape=snow, color=water, y=precip_est_mm))+geom_point()+ 
  facet_wrap(vars(year), nrow=1) + scale_color_manual(values=water_pal) + scale_y_continuous(limits=c(0,NA))+
  labs(y="Estimated summer precipiation with treatments (mm)", x="Snowmelt", color="Precipitation", shape="Snowmelt")
```

The timing of measurements and treatments differed between years
```{r timings}
ggplot(timings %>% mutate(variable=fct_reorder(variable, begin, na.rm=T, .desc=T)), aes(y=variable, color=year))+ 
  ...
  scale_y_discrete("", labels=c(snowcloth="Snow cloths applied to early plots", meltdates="Mean snowmelt timing (early plots - unmanipulated plots)", summer_precip_timing="Summer precipitation summation period", waterdates="Summer precipitation treatments applied",sm="Soil moisture recorded",ph="Inflorescence height recorded", mt="Floral morphology recorded", nt="Nectar traits recorded", sds="Flower collection period"))+
...
ph %>% drop_na(height_cm) %>% group_by(year, plantid) %>% tally() %>% 
  ggplot(aes(x=n, fill=year)) + geom_histogram() + 
...
ggplot(mt, aes(x=nectar_24_h_ul, y = nectar_conc)) + geom_point() + labs(x="Nectar Production Rate (uL / day)", y="Nectar concentration (% sucrose by mass)") + geom_smooth(se=F)
```
...
mt.mod.raw <- mt.mod.plantyr <- mt.mod.subplot <- mt.mod.subplot.test <- mt.mod.plot <- coef.melt.precip <-emm.melt<- emm.precip <- list()
for(tr in traits) {
  #mt.mod.raw[[tr]]     <- lmer(mt[[tr]]         ~ year * water * melt_date + (1|plotid) + (1|plantid), data=mt)
  #mt.mod.plantyr[[tr]] <- lmer(mt.plantyr[[tr]] ~ year * water * melt_date + (1|plotid), data=mt.plantyr)
...
ggplot(emm.melt.df, aes(x=fct_relevel(trait, rev(traits)), y=trend)) +  
  ...
  theme_classic() + scale_x_discrete(labels = traitnames) + coord_flip() + scale_color_manual(values=water_pal) +
...
for(tr in traits) {
  ...
  mnps.plantyr[[paste0("fit.sm.",tr)]][!is.na(mnps.plantyr[[tr]])]<- predict(mt.sm.mod[[tr]], re.form=NA)
  mt.sm.mod.test[[tr]] <- anova(mt.sm.mod[[tr]])
  
  #Z-score the data first to get standardized slopes
  mt.sm.mod[[tr]] <- lmer(scale(mnps.plantyr[[tr]]) ~ year * VWC + (1|plotid), data=mnps.plantyr)
...
bind_rows(melt_date = emm.melt %>% map(as.data.frame) %>% bind_rows(.id="trait") %>% rename(trend=melt_date.trend),
          early_precip_mm = emm.precip %>% map(as.data.frame) %>% bind_rows(.id="trait") %>% rename(trend=early_precip_mm.trend),      .id="predictor") %>% mutate(predictor=fct_relevel(predictor,"melt_date")) %>%
  filter(trait %in% traits[c(1:8, 13)]) %>% 
  ggplot(aes(x=fct_relevel(trait, rev(traits)), y=trend, color=water)) +  
  facet_wrap(vars(predictor), nrow=1, scales="free_x", labeller=labeller(predictor=c(early_precip_mm="Early summer precip (mm)", melt_date="Snowmelt date"))) + 
  geom_hline(yintercept=0) + 
  geom_pointrange(aes(ymin=trend - SE, ymax = trend + SE), position=position_dodge(width=0.3), size=0.4) + 
  labs(y="Slope of z-scored trait vs. predictor", x="Floral Trait", color="Water") +
  theme_classic() + scale_x_discrete(labels = traitnames) + coord_flip() + scale_color_manual(values=water_pal) +
