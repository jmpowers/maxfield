...
ggplot(mapping=aes(x=CAP1, y=CAP2, label=Label)) + 
  ...
  geom_text(data=cap.df %>% filter(Score=="biplot") %>% 
              mutate(Label=c("Snowmelt", "Precipitation", "Snowmelt x Precipitation")),
...
ggplot(volsmeta, aes(x = sun_date, y = total)) + 
  geom_point(aes(fill=precip_est_mm, shape=year), color="white", size=2) + 
  geom_smooth(aes(color=water), se=F)+ theme_minimal()+ scale_y_sqrt()  + 
  labs(x="Snowmelt date", y ="Total filtered emissions (ng/hr/flower)", shape="Year", color="Precipitation") +
  water_scale + year_scale + scale_color_manual(values=water_pal)
```
...
#   labs(x="Summer precipitation (mm)", y ="Emissions (ng/hr/flower)", shape="Year", color="Snowmelt") +
...
ggplot(volsmeta, aes(x = precip_est_mm, y = total)) + 
  geom_point(aes(fill=sun_date, shape=year), color="white", size=2) + 
  geom_smooth(aes(color=snow), se=F)+ theme_minimal()+ scale_y_sqrt()  + 
  labs(x="Summer precipitation (mm)", y ="Total filtered emissions (ng/hr/flower)", shape="Year", color="Snowmelt") +
  snow_scale + year_scale + scale_color_manual(values=snow_pal)
```
...
walk(fitnesstraits, ~ 
       print(ggplot(traits.good.top %>% filter(ifelse(.x == "prop_infested", fruits_nonaborted > 4, TRUE)), 
...
```{r selection_class}
walk(fitnesstraits, ~ 
       print(ggplot(traits.class %>% filter(ifelse(.x == "prop_infested", fruits_nonaborted > 4, TRUE)), 
                    aes(x=value, color=paste(water,snow), linetype=year, shape=year)) + 
               geom_point(aes_string(y=.x)) + geom_smooth(aes_string(y=.x), method="lm",se=F) + 
               facet_wrap(vars(name), scales="free_x") + 
               labs(x="Emissions (ng/hr/flower)", y=fitnessnames[.x], 
