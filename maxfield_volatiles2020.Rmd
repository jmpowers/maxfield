...
selection_pca <- function(year.cluster, water) {
vols.std <- bind_cols(volsmeta, vols.good.top) %>% 
  filter(year.cluster == .env$year.cluster, water %in% .env$water) %>% 
...
plot_beta <- function(betas, colorby="year.cluster") {
...
vols.beta.yr <- map_dfr(set_names(unique(volsmeta$year.cluster)), 
                     ~ selection_pca(.x, names(water_pal)), .id="year.cluster")
plot_beta(vols.beta.yr)

vols.beta.water <- expand_grid(year.cluster=unique(volsmeta$year.cluster), water=names(water_pal))  %>% 
  mutate(betas = pmap(., selection_pca)) %>% unnest(betas)
plot_beta(filter(vols.beta.water, year.cluster=="2018"), colorby="water") + 
  scale_color_manual(values=water_pal) + ggtitle("2018")

plot_beta(filter(vols.beta.water), colorby="water") + 
  scale_color_manual(values=water_pal) + facet_wrap(vars(year.cluster), nrow=1)
```
