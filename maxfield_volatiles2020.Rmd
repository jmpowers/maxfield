...
```{r selection_class_gradient, fig.width=9, fig.height=7}
...
walk(adaptive.plasticity.plots, 
     ~ print(.x + facet_grid(rows=vars(year), cols=vars(fitnesstrait), scales="fixed", 
               ...
               #geom_smooth(aes(y=B, group=1), color="grey30", method="lm", se=F, size=1, formula= y ~ x + 0)+
               ...
               #stat_cor(aes(y=B), size=3, label.x=0.1, r.accuracy=0.01, p.accuracy = 0.01)+
               scale_size_manual(values=c(1,2), labels=c("NS", "Signif."))+
               scale_x_continuous(labels=scales::percent, limits=c(NA,NA), breaks=c(-6:6)/2) + 
...
bind_rows(added_precip=plasticity.water %>% left_join(vols.beta.water %>% rename(name=trait) %>%
                                               filter(water=="Addition", fitnesstrait!="eggs_per_flower")) %>% 
            mutate(adaptive = sign(B) == sign(A_C)),
          reduced_precip=plasticity.water %>% left_join(vols.beta.water %>% rename(name=trait) %>%
                                               filter(water=="Reduction", fitnesstrait!="eggs_per_flower")) %>% 
            mutate(adaptive = sign(B) == sign(R_C)),
          early_snowmelt=plasticity.snow %>% left_join(vols.beta.snow %>% rename(name=trait) %>% 
                                              filter(snow=="Early", fitnesstrait!="eggs_per_flower")) %>% 
            mutate(adaptive = sign(B) == sign(E_N)), .id="environchange") %>% 
  group_by(environchange, fitnesstrait) %>% summarize(adaptive=sum(adaptive), n=n(), .groups="drop") %>% 
...
```{r snow_v_water, fig.height=6, fig.width=4}
...
library(ggrepel)
ggplot(mod.selection.tests, aes(x=response, y=estimate, color=class, label=greekify(trait))) + 
  ...
  labs(x="Aspect of fitness", size="Mean emissions\n(ng/hr/flower)", color="Class") + 
