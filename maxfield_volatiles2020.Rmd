...
```{r averaging}
volsmeta %>% count(year, plantid, name="n_samples") %>% count(n_samples)
...
vols.cut.avg <- vols.cut %>% bind_cols(volsmeta) %>% 
  group_by(across(all_of(c("plantid", colnames(treatments))))) %>% 
  summarize(across(all_of(colnames(vols.cut)), ~mean(.x, na.rm=T)), .groups="drop") %>% 
...
vols.good.avg <- vols.good %>% bind_cols(volsmeta) %>% 
  ...
  select(-any_of(colnames(volsmeta)))
rownames(volsmeta.avg) <- rownames(vols.cut.avg) <- rownames(vols.good.avg) <- volsmeta.avg$index
```


```{r topvols}
minsamples <- nrow(vols.good) * 0.65
