...
library(dendsort)
sequence.date_pal <- list(sequence.date = volsmeta %>% mutate(yr = year(sequence.date)) %>% group_by(yr) %>% summarize(n = length(unique(sequence.date))) %>% pull(n) %>% map(scales::hue_pal()) %>% unlist() %>% set_names(unique(volsmeta$sequence.date)))
...
pheatmap(as.matrix(t(vols.cut))^(1/4), 
         ...
         annotation_col=volsmeta %>% mutate(sequence.date = factor(sequence.date)) %>% select(sequence.date),
         annotation_colors = sequence.date_pal,
...
vols.plot <- ggplot(vols.nmds.samples, aes(x=NMDS1, y=NMDS2)) + theme_classic() +
  geom_point(aes(color=factor(year(sequence.date)))) + 
