...
```{r gradients, fig.width=9, fig.height=9}
...
selection_pca <- function(year, water=names(water_pal), snow=names(snow_pal), fitnesstrait = "seeds_est", 
                          ...
                          pca = vols.PCA, mdata = volsmeta.avg.cut) {
  if(ncol(pca$x) <= n.PCs) {
...
plot_beta <- function(betas, colorby="year") {
  ...
  ggplot(bs, aes(y=trait, x = B, xmin=B-SE, xmax=B+SE, alpha=p.value<0.05, shape=year)) + 
    ...
    labs(x = "Selection gradient (beta) estimated from gradients on PCs", y = "Trait", shape="Year",
...
mod.selection.abs <- expand.grid(list(response = fitnesstraits, trait = c(ipogood, traits[c(1:6,8)]))) %>%
...
ggplot(mod.selection.tests, aes(x=factor(response, levels=sort(fitnesstraits)), y=estimate, label=greekify(trait))) + 
  ...
  scale_color_manual(values=c(class_pal,"Other trait"="black"), guide=guide_legend(override.aes = list(size=5))) + 
...
pca <- combos$vmn %>% mutate(across(all_of(ipogood.top), sqrt)) %>% prcomp_std()

pca.vars <- as.data.frame(pca$rotation) %>% rownames_to_column("variable") %>% 
  mutate(class=fct_collapse(variable, !!!deframe(select(filter(ipochemsf, name %in% ipogood.top), class, name)), 
                            other_level="Other trait")) 
pca.labs <- paste0("PC", 1:2, " (", round(100*pca$sdev[1:2]^2/sum(pca$sdev^2), 0), "% explained)") 
zoom<-15
ggplot(bind_cols(combos$vmn, as_tibble(pca$x)), aes(x=PC1, y=PC2)) +
  geom_point(shape=21, color="grey80", size=2) + #, fill=`a-pinene` + scale_fill_viridis_c(option = "plasma")
  geom_segment(data=pca.vars, aes(x=0, y=0, xend=PC1*zoom, yend=PC2*zoom), 
               arrow = arrow(length = unit(1/2, "picas")), color="grey80") +
  geom_text(data=pca.vars, show.legend = F,
  ...
  geom_point(data=pca.vars, aes(color=class), alpha=0)+ 
  guides(color=guide_legend(override.aes = list(alpha=1, size=3)))+
  labs(color="", x=pca.labs[1], y=pca.labs[2]) 
```
...
volsmeta.avg.cut <- #treatments used in selection_pca function
  volsmeta.avg[!is.na(rowSums(vols.traits[,colnames(combos$vmn)])),] #match rows in vmn

#TODO consider sqrt transformation: https://bcrc.bio.umass.edu/projects/selection_analysis/Main.htm
vols.PCA <- combos$vmn %>% mutate(across(all_of(ipogood.top), sqrt)) %>% prcomp_std()
...
combos.PCA <- map(combos,  ~ .x %>% mutate(across(any_of(ipogood.top), sqrt)) %>% prcomp_std())
