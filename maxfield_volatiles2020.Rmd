...
mnps.plantyr.traits %>% select(all_of(fitnesstraits)) %>% 
  ggpairs(mapping=aes(color=mnps.plantyr.traits$year), lower=list(continuous="smooth_loess")) + 
...
adaptive.plasticity.data %>% filter(newenv!="R_A") %>% 
  ...
  pwalk(function(newenv, data) { 
    print(ggplot(data, aes(x=estimate, y=B, color=class)) + 
            ...
            geom_point(aes(size=mean.floral)) + 
            scale_size_area("Mean emissions\n(ng/hr/flower)", max_size=3) +
            scale_x_continuous(labels=scales::percent, limits=c(NA,NA), breaks=c(-4:4)) + 
...
ggplot(adaptive.plasticity.data %>% filter(newenv!="R_A"), aes(x=estimate, xmin=estimate-std.error, xmax=estimate+std.error,
                          y=name, color=year, alpha=1-adj.p.value)) +  facet_wrap(vars(newenv)) +
...
library(ggrepel)
ggplot(mod.selection.tests, aes(x=factor(response, levels=rev(fitnesstraits)), y=estimate, label=greekify(trait))) + 
