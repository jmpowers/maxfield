...
```{r sm}
ggplot(vt %>% drop_na(water), aes(x=factor(sampledate), y=VWC, color=water)) + facet_wrap(vars(year), nrow = 1, scales="free_x") +
...
mnps.plantyr.traits <- mnps.plantyr %>% select(year, plotid, plant, all_of(traits), eggs_per_flower)
...
ggplot(volsmeta.good.top, aes(x = sun_date, y = value)) + facet_wrap(vars(name), scales="free_y") + 
  ...
  labs(x="Snowmelt date", y ="Emissions (ng/hr)", shape="Year", color="Precipitation") +
...
```{r vols_abs_precip, fig.width=16, fig.height=10}
snow.breaks <- seq(110,160, by=10)
snow_scale <- scale_fill_gradientn("Snowmelt date", colors=snow_pal, 
                        values=rev(c(1, (treatments %>% group_by(snow) %>% summarize(across(sun_date, mean)) %>%
                                           pull(sun_date) %>% scales::rescale(from=range(snow.breaks)))[2],0)),
                        breaks=snow.breaks)
year_scale <- scale_shape_manual(values=21:23, guide=guide_legend(override.aes=list(fill="black"))) 

ggplot(volsmeta.good.top, aes(x = precip_est_mm, y = value)) + facet_wrap(vars(name), scales="free_y") + 
  geom_point(aes(fill=sun_date, shape=year), color="white", size=2) + 
  geom_smooth(aes(color=snow), se=F)+ theme_minimal()+ scale_y_sqrt()  + 
  labs(x="Summer precipitation (mm)", y ="Emissions (ng/hr)", shape="Year", color="Snowmelt") +
  snow_scale + year_scale + scale_color_manual(values=snow_pal)

ggplot(volsmeta.class, aes(x = precip_est_mm, y = value)) + facet_wrap(vars(name), scales="free_y") + 
  geom_point(aes(fill=sun_date, shape=year), color="white", size=2) + 
  geom_smooth(aes(color=snow), se=F)+ theme_minimal()+ scale_y_sqrt()  + 
  labs(x="Summer precipitation (mm)", y ="Emissions (ng/hr)", shape="Year", color="Snowmelt") +
  snow_scale + year_scale + scale_color_manual(values=snow_pal)

ggplot(volsmeta, aes(x = precip_est_mm, y = total)) + 
  geom_point(aes(fill=sun_date, shape=year), color="white", size=2) + 
  geom_smooth(aes(color=snow), se=F)+ theme_minimal()+ scale_y_sqrt()  + 
  labs(x="Summer precipitation (mm)", y ="Total filtered emissions (ng/hr)", shape="Year", color="Snowmelt") +
  snow_scale + year_scale + scale_color_manual(values=snow_pal)
```
...
(scents.cap <- capscale(sqrt(vols.good[hasVWC,]) ~ year * VWC, data=volsmeta[hasVWC,]))
anova(scents.cap, by="term") %>% kable()
```
...
```{r fitness_corr}
library(GGally)
mnps.plantyr %>% select(flowers_est, seeds_est, seeds_per_flower, prop_infested) %>% 
...
```{r selection_spf, fig.width=16, fig.height=10}
ggplot(traits.good.top, aes(x=value, y=seeds_per_flower, color=year)) + 
  geom_point() + geom_smooth(method="lm", se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Seed per flower", color="Year")+
  theme_bw()+ scale_x_sqrt()+
  scale_color_manual(values=year_pal)

ggplot(traits.good.top, aes(x=value, y=seeds_per_flower, color=water, linetype=year, shape=year)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Seed per flower", color="Precipitation", linetype="Year", shape="Year")+
  theme_bw() + scale_color_manual(values=water_pal)+ scale_x_sqrt()

ggplot(traits.good.top, aes(x=value, y=seeds_per_flower, color=snow, linetype=year, shape=year)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Seed per flower", color="Snowmelt", linetype="Year", shape="Year")+
  theme_bw() + scale_color_manual(values=snow_pal)+ scale_x_sqrt()
```

```{r selection_class_spf}
ggplot(traits.class, aes(x=value, y=seeds_per_flower, color=water, linetype=year, shape=year)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Seed per flower", color="Precipitation", linetype="Year", shape="Year")+
  theme_bw() + scale_color_manual(values=water_pal)+ scale_x_sqrt()

ggplot(traits.class, aes(x=value, y=seeds_per_flower, color=snow, linetype=year, shape=year)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Seed per flower", color="Snowmelt", linetype="Year", shape="Year")+
  theme_bw() + scale_color_manual(values=snow_pal)+ scale_x_sqrt()

ggplot(traits.class, aes(x=total, y=seeds_per_flower, color=water, linetype=year, shape=year)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  labs(x="Total emissions", y="Seed per flower", color="Precipitation", linetype="Year", shape="Year")+
  theme_bw() + scale_color_manual(values=water_pal)+ scale_x_sqrt()

ggplot(traits.class, aes(x=total, y=seeds_per_flower, color=snow, linetype=year, shape=year)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  labs(x="Total emissions", y="Seed per flower", color="Snowmelt", linetype="Year", shape="Year")+
  theme_bw() + scale_color_manual(values=snow_pal)+ scale_x_sqrt()
```
...
ggplot(traits.good.top, aes(x=value, y=eggs_per_flower, color=year)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Mean fly eggs / mean flowers", color="Year")+
...
ggplot(traits.class, aes(x=value, y=eggs_per_flower, color=year)) + 
  geom_point() + geom_smooth(method="loess",span=0.8,se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Mean fly eggs / mean flowers")+
  theme_bw()+ scale_x_sqrt() + ylim(0,1.2)+  scale_color_manual(values=year_pal)
...
ggplot(traits.class, aes(x=total, y=eggs_per_flower, color=year)) + 
  geom_point() + geom_smooth(method="loess",span=0.8,se=F) + 
  labs(x="Emissions", y="Mean fly eggs / mean flowers", color="Year")+
  theme_bw()+  scale_x_sqrt() + ylim(0,1.2)+  scale_color_manual(values=year_pal)
```
...
fitness.matrix <- volsmeta.traits %>% select(seeds_est, seeds_per_flower, prop_infested, eggs_per_flower)
...
tibble(name = c("seeds_est", "seeds_per_flower", "prop_infested", "eggs_per_flower"), SR2 = MF$SR2) %>% 
...
selection_pca <- function(year, water=names(water_pal), snow=names(snow_pal)) {
...
print(summary(vols.PCA)) # PCs 1 - 5 explain 51% of the variation
vols.PCA.E <- vols.PCA$rotation[,1:min(4, ncol(vols.PCA$rotation))] #TODO otherwise fails at selection_pca(2020,"Addition","Normal" bc small sample size?
vols.std.PCA <- bind_cols(vols.std, as.data.frame(vols.PCA$x))

# Regress relative fitness on PCs
vols.PCA.regression <- lm(seeds_rel ~ PC1 + PC2 + PC3 + PC4, data = vols.std.PCA)
...
plot_beta <- function(betas, colorby="year") {
  bs <- betas %>% mutate(trait = fct_reorder(trait, B))
...
# 
# vols.beta.snow <- expand_grid(year=unique(volsmeta$year), snow=names(snow_pal))  %>% 
#   mutate(betas = pmap(., selection_pca)) %>% unnest(betas)
# 
# plot_beta(filter(vols.beta.snow), colorby="snow") + facet_wrap(vars(year), nrow=1)

vols.beta.water.snow <- expand_grid(year=unique(volsmeta$year), water=names(water_pal), snow=names(snow_pal))  %>% 
  mutate(betas = pmap(., selection_pca)) %>% unnest(betas)

plot_beta(filter(vols.beta.water.snow), colorby="water_snow") + facet_wrap(vars(year), nrow=1)
```

## Seeds per flower

```{r gradients_spf, fig.width=14, fig.height=10}
calculate_beta <- function(E, slopes) {
  data.frame(B = E %*% slopes$A,
             SE = sqrt(E ^ 2 %*% slopes$SE ^ 2)) %>% rownames_to_column("trait")
}
...
vols.std <- bind_cols(volsmeta, vols.good.top) %>% 
  filter(year == .env$year, water %in% .env$water, snow %in% .env$snow) %>% 
  left_join(mnps.plantyr %>% select(year, plotid, plant, seeds_per_flower)) %>% 
  select(all_of(ipogood.top),seeds_per_flower, any_of(names(treatments))) %>% 
  mutate(across(all_of(ipogood.top), ~ (.x-mean(.x, na.rm=T))/sd(.x, na.rm=T))) %>% 
  group_by(year) %>% mutate(seeds_rel = seeds_per_flower/mean(seeds_per_flower, na.rm=T), .keep="unused") %>% ungroup %>% 
  drop_na(seeds_rel) %>% 
  select(where(~sum(is.na(.))==0))  #drop_na(all_of(ipogood.top)) 
...
selection_mod_abs <- function(trait, response) {
  vm.abs <- bind_cols(volsmeta.traits, vols.good) %>% 
    ...
    mutate(response = response/sd(response), trait = trait/sd(trait))
  options(contrasts = c("contr.sum", "contr.poly"))
  lm(response ~ trait * sun_date + trait * precip_est_mm, data = vm.abs)
}
fitnesstraits <- c("seeds_est", "seeds_per_flower", "prop_infested", "eggs_per_flower")
...
ggplot(mod.selection.tests, aes(x=response, y=estimate, color=class, label=trait)) + 
  facet_wrap(vars(term), labeller = as_labeller(c(trait="Trait (SD fitness/ SD emissions)",
                                                  `trait:precip_est_mm`=
                                                    "Trait x summer precipitation (SD fitness / SD emissions / mm)",
                                                  `trait:sun_date`=
                                                    "Trait x snowmelt date (SD fitness / SD emissions / day)")),
             scales="free_y", ncol=1) + theme_bw() +
  geom_point(aes(size=mean.floral)) + scale_size_area()+
  geom_hline(yintercept=0) + 
  geom_text(aes(alpha = p.value < 0.05), hjust=0, nudge_x = 0.1) + scale_alpha_discrete(range=c(0,1), guide="none") + 
  labs(x="Fitness component", y = "Estimate", size="Mean emissions (ng/hr)", color="Class") + 
  scale_x_discrete(labels=c("Total seeds", "Seeds per flower", "Fly fruits", "Fly eggs per flower"))
