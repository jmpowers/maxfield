...
library(ggpubr)
adaptive.plasticity.data %>% group_by(newenv) %>% nest() %>% 
...
ggplot(adaptive.plasticity.data, aes(x=estimate, xmin=estimate-std.error, xmax=estimate+std.error,
...
adaptive.plasticity.data %>% 
  mutate(adaptive = sign(B) == sign(estimate)) %>% 
