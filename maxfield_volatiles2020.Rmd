...
fitnessnames <- c(traitnames, eggs_per_flower="Fly eggs per flower")[fitnesstraits]
```
...
#TODO run for all fitness metrics
# used to split the data before running PCA - better to run PCA on whole dataset
...
selection_pca <- function(year, water=names(water_pal), snow=names(snow_pal), fitnesstrait = "seeds_est") {
  vols.std.PCA <- volsmeta %>% bind_cols(as.data.frame(vols.PCA$x)) %>% 
    ...
    left_join(mnps.plantyr %>% select(year, plotid, plant, all_of(fitnesstrait))) %>% 
...
```{r adaptive, fig.height=8, fig.width=10}
...
adaptive.plasticity.plots <- list(
  plasticity.water %>% left_join(vols.beta.water %>% rename(name=trait) %>% filter(water=="Addition")) %>% 
    ggplot(aes(x=A_C)) + labs(x="Emissions change with additional precipitation", 
                              y="Selection gradient (beta) under additional precipitation"),
  plasticity.water %>% left_join(vols.beta.water %>% rename(name=trait) %>% filter(water=="Reduction")) %>% 
    ggplot(aes(x=R_C)) + labs(x="Emissions change with reduced precipitation", 
                              y="Selection gradient (beta) under reduced precipitation"),
  plasticity.snow %>% left_join(vols.beta.snow %>% rename(name=trait) %>% filter(snow=="Early")) %>% 
    ggplot(aes(x=E_N)) + labs(x="Emissions change with early snowmelt", 
                              y="Selection gradient (beta) under early snowmelt"))

walk(adaptive.plasticity.plots, 
     ~ print(.x + facet_grid(rows=vars(year), cols=vars(fitnesstrait), scales="free_y", 
                             labeller=as_labeller(c(fitnessnames, set_names(levels(treatments$year)))))+
               geom_vline(xintercept=0) + geom_hline(yintercept=0) + geom_point(aes(y=B, color=class, size=mean.floral)) + 
               scale_size_area("Mean emissions\n(ng/hr/flower)", max_size=10) + scale_x_continuous(labels=scales::percent) + 
               scale_color_manual("Compound class", values=class_pal, guide=guide_legend(override.aes = list(size=5))) + theme_bw()))
```
