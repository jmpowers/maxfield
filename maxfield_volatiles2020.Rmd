...
selection_pca <- function(year, water=names(water_pal), snow=names(snow_pal), fitnesstrait = "seeds_est", 
                          n.PCs = 5, #default number of PCs in regression
                          PCA = pca, mdata = meta.pca) {
  if(ncol(PCA$x) <= n.PCs) {
...
(floraltraits.pca <- floraltraits[-c(4,8)])  #drop sepal_width - not measured in 2018, drop total flowers
mnps.PCA <- mnps.plantyr %>% select(all_of(floraltraits.pca)) %>% drop_na() %>% prcomp_std()
summary(mnps.PCA) # PCs 1 - 4 explain 83% of the variation, 6 PCs total

meta.pca <- mnps.plantyr %>% filter(mnps.plantyr %>% select(all_of(floraltraits.pca)) %>% vctrs::vec_detect_complete())
pca <- mnps.PCA
mnps.beta.yr <- expand_grid(year=unique(mnps.plantyr$year), fitnesstrait=fitnesstraits, n.PCs = 4)  %>% 
 mutate(betas = pmap(., selection_pca)) %>% unnest(betas)
mnps.beta.water <- expand_grid(year=unique(mnps.plantyr$year), water=names(water_pal), fitnesstrait=fitnesstraits, n.PCs = 4)  %>% 
 mutate(betas = pmap(., selection_pca)) %>% unnest(betas)
mnps.beta.snow <- expand_grid(year=unique(mnps.plantyr$year), snow=names(snow_pal), fitnesstrait=fitnesstraits, n.PCs = 4)  %>% 
 mutate(betas = pmap(., selection_pca)) %>% unnest(betas)
mnps.beta.water.snow <- expand_grid(year=unique(mnps.plantyr$year), water=names(water_pal), snow=names(snow_pal), 
                                    fitnesstrait=fitnesstraits, n.PCs = 4)  %>% 
  mutate(betas = pmap(., selection_pca)) %>% unnest(betas)
```
...
#By year
plot_beta(bind_rows(vols.beta.yr, mnps.beta.yr)) + 
  facet_wrap(vars(fitnesstrait), nrow=1, labeller=as_labeller(fitnessnames))

#By water and snow
plot_beta(bind_rows(vols.beta.water.snow, mnps.beta.water.snow), colorby="water_snow") +
  facet_wrap(vars(fitnesstrait), nrow=1, labeller=as_labeller(fitnessnames)) + coord_cartesian(xlim=c(-2,3.5))

# walk(fitnesstraits, ~print(plot_beta(filter(bind_rows(vols.beta.water.snow, mnps.beta.water.snow), fitnesstrait==.x), colorby="water_snow") + 
#                              facet_wrap(vars(year), nrow=1) + ggtitle(fitnessnames[.x])))
```

```{r gradient_heatmap, fig.width=11}
#By year
bind_rows(vols.beta.yr, mnps.beta.yr) %>% left_join(ipochemsf, by=c("trait"="name")) %>% 
...
#by water and snow
bind_rows(vols.beta.water.snow, mnps.beta.water.snow) %>% left_join(ipochemsf, by=c("trait"="name")) %>% 
        mutate(trait = recode(trait, !!!traitnames) %>% fct_reorder(replace_na(mean.floral,1000))) %>% 
ggplot(aes(x=paste(water, snow, year), y=trait, fill=B)) + geom_tile() + 
  scale_fill_gradient2()+ geom_point(aes(alpha=p.value < 0.05))+ scale_alpha_discrete(range=c(0,1))+ labs(x="")+
  facet_wrap(vars(fitnesstrait), nrow=1, labeller=as_labeller(fitnessnames)) + theme(axis.text.x = element_text(angle=90, hjust=0))

#TODO: try modeling gradients against continuous water and snow, plot like plasticity graph 
```
...
adaptive.plasticity.data <- volsmeta.good.top %>% group_by(name, year) %>% nest() %>% 
  ...
  left_join(bind_rows(rename(vols.beta.water, newenv=water),
                    rename(vols.beta.snow, newenv=snow)) %>% 
...
adaptive.plasticity.data %>% filter(newenv!="R_A") %>% 
  ...
  pwalk(function(newenv, data) { 
    print(ggplot(data, aes(x=estimate, y=B, color=class)) + 
            ...
            scale_x_continuous(labels=scales::percent, breaks=c(-4:4)) + 
...
adaptive.plasticity.data %>% 
  ...
  kable(caption=paste("Proportion of", ncol(vols.good.top), "volatiles whose plasticity is adaptive across 3 yr"), digits=2)
```
...
ggplot(compare.plasticity, aes(x=R_A, xmin=R_A-R_A.SE, xmax=R_A+R_A.SE, 
  ...
  geom_errorbar() + geom_errorbarh() + geom_point(aes(size=mean.floral)) + 
...
```{r selection_abs, fig.width=8, fig.height=7}
...
```{r vmnt_corr}
library(corrplot)
corrplot(cor(combos$vmn), method="color", type="lower",
         order = "hclust", hclust.method = "complete", addrect = 4, 
...
ggplot(vols.beta.yr.PCs, aes(x=n.PCs)) + facet_wrap(vars(fitnesstrait), scales="free_y") + 
  ...
  labs(color="") #+ geom_vline(xintercept=5) + ylim(c(-0.66,1)) + 
  #geom_line(data=tibble(cumul_expl=summary(vols.PCA)$importance[3,]) %>% mutate(n.PCs = 1:nrow(.)),
  #          aes(y=cumul_expl), color="black")
```
