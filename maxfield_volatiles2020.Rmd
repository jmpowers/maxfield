...
# Run PCA on traits
vols.PCA <- vols.good.top %>% 
  mutate(across(everything(), ~ (.x-mean(.x, na.rm=T))/sd(.x, na.rm=T))) %>% #center and scale
  prcomp() 
...
selection_pca <- function(year, water=names(water_pal), snow=names(snow_pal), fitnesstrait = "seeds_est", 
                          n.PCs = 5) {#defualt number of PCs used in regression
  std.traits <- volsmeta.avg.cut %>% bind_cols(as.data.frame(vols.PCA$x)) %>% 
  ...
  calculate_beta(E = vols.PCA$rotation[,1:n.PCs],
...
# drop plants with missing data for 3 combinations of measurements
...
vols.traits %>% 
  mutate(v=vctrs::vec_detect_complete(.[,c(ipogood.top)]),
         vm = vctrs::vec_detect_complete(.[,colnames(vm)]),
         vn = vctrs::vec_detect_complete(.[,colnames(vn)]),
         vmn = vctrs::vec_detect_complete(.[,colnames(vmn)])) %>% 
  group_by(year, water, snow) %>% 
  summarize(across(c(v, vm, vn, vmn), sum)) %>% kable(caption="plants with each set of measurements")
```
...
```{r vmnt_corr}
library(corrplot)
corrplot(cor(vmn), method="color", type="lower",
...
pca <- vmn %>% mutate(across(all_of(ipogood.top), sqrt)) %>% 
    mutate(across(everything(), ~ (.x-mean(.x, na.rm=T))/sd(.x, na.rm=T))) %>% #center and scale
...
zoom<-15
ggplot(bind_cols(vmn, as_tibble(pca$x)), aes(x=PC1, y=PC2)) +
...
volsmeta.avg.cut <- #treatments used in selection_pca function
  volsmeta.avg[!is.na(rowSums(vols.traits[,colnames(vmn)])),] #match rows in vmn

#TODO consider sqrt transformation: https://bcrc.bio.umass.edu/projects/selection_analysis/Main.htm
vols.PCA <- vmn %>% mutate(across(all_of(ipogood.top), sqrt)) %>% 
    mutate(across(everything(), ~ (.x-mean(.x, na.rm=T))/sd(.x, na.rm=T))) %>% #center and scale
  prcomp(center = F, scale = F)
...
Black curve is cumulative proportion explained by PCs. Black line is 5 PCs.

```{r numberPCs}
vols.beta.yr.PCs <- expand_grid(year=unique(volsmeta$year), fitnesstrait=fitnesstraits, 
                                n.PCs=2:17)  %>% 
  mutate(betas = pmap(., selection_pca)) %>% unnest(betas)
ggplot(vols.beta.yr.PCs, aes(x=n.PCs)) + facet_wrap(vars(fitnesstrait)) + 
