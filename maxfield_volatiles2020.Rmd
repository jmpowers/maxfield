...
selection_mod_abs <- function(trait, response) {
  vm.abs <- bind_cols(volsmeta.traits, vols.good.avg) %>% 
...
ggplot(mod.selection.tests, aes(x=factor(response, levels=sort(fitnesstraits)), y=estimate, label=greekify(trait))) + 
  ...
  geom_point(aes(size=mean.floral, color=class)) + geom_hline(yintercept=0) + 
  geom_text_repel(data = filter(mod.selection.tests, p.value < 0.05) %>% 
                    mutate(trait=recode(trait, "(1Z,4Z,7Z)-1,5,9,9-tetramethylcycloundeca-1,4,7-triene"=
                                          "(1Z,4Z,7Z)-1,5,9,9-\ntetramethylcycloundeca-1,4,7-triene")), 
...
Venn for the plants that have seeds counted
...
```{r vmnt_pca}
vols.traits <- bind_cols(volsmeta.avg, vols.good.avg) %>% full_join(mnps.plantyr.traits)

vmnt <- vols.traits %>% select(corolla_length, corolla_width, style_length, 
                               nectar_24_h_ul, nectar_conc, height_cm, all_of(ipogood)) %>% 
  drop_na() %>% #drop plants with any missing data
  mutate(across(all_of(ipogood), sqrt))

vmnt.pca <- prcomp(vmnt, scale.=TRUE)
vmnt.morph.pca <- vmnt %>%
  bind_cols(as.data.frame(vmnt.pca$x)) %>% 
...
vmnt.pca.vars <- as.data.frame(vmnt.pca$rotation) %>% rownames_to_column("variable") %>% 
  mutate(class=fct_collapse(variable, !!!deframe(select(ipochemsf, class, name)), other_level="Other trait")) 
vmnt.pca.labs <- paste0("PC", 1:2, " (", round(100*vmnt.pca$sdev[1:2]^2/sum(vmnt.pca$sdev^2), 0), "% explained)") 
zoom<-15
ggplot(vmnt.morph.pca, aes(x=PC1, y=PC2)) +
  geom_point(shape=21, color="grey80", size=2) + #, fill=`a-pinene`
  geom_segment(data=vmnt.pca.vars, aes(x=0, y=0, xend=PC1*zoom, yend=PC2*zoom), 
               arrow = arrow(length = unit(1/2, "picas")), color="grey80") +
  geom_text(data=vmnt.pca.vars, aes(label = greekify(recode(variable, !!!traitnames)), 
                                   x = PC1*zoom*1.1, y = PC2*zoom*1.1, color = class,
                                   angle =  (180/pi) * atan(PC2/PC1), hjust=if_else(PC1>0,0,1)), size=3, show.legend = F)+
  coord_fixed()+ scale_fill_viridis_c(option = "plasma")+ scale_color_manual(values=c(class_pal, "Other trait"="black")) + 
  theme_minimal()+ theme(panel.grid=element_blank(), legend.position = "top") + 
  geom_point(data=vmnt.pca.vars, aes(color=class), alpha=0)+ 
  guides(color=guide_legend(override.aes = list(alpha=1, size=3)))+
  labs(color="", fill="sqrt(a-pinene)", x=vmnt.pca.labs[1], y=vmnt.pca.labs[2]) 
```

```{r vmnt_corr}
library(corrplot)
corrplot(cor(vmnt), method="color", type="lower",
