...
sort(with(volsmeta, sequence.date - sampledate))
```
...
volsmeta %>% count(year, sampledate) %>% filter(n>1) %>% count(year) %>% kable(caption="More than one sample taken on these dates")
```
...
```{r cap}
(scents.cap <- capscale(sqrt(vols.good) ~ year.cluster * water * snow, data=volsmeta))
anova(scents.cap, by="term")

scents.cap <- capscale(sqrt(vols.good) ~ year.cluster + water, data=volsmeta)
plot(scents.cap, type="n")
points(scents.cap, display = "sites", col=water_pal[volsmeta$water], pch=as.integer(volsmeta$year)+14)
text(scents.cap, display="cn", labels=c(unique(volsmeta$year.cluster), names(water_pal)), col=c(rep("black",5),water_pal), font=2, cex=1.5)
...
model.abu <- Hmsc(Y=vols.good,
                XData = volsmeta %>% select(year.cluster, water, snow) %>% mutate_all(as.factor), 
                XFormula= ~ year.cluster + water + snow, 
...
#save(model.abu, file="./data/volatiles/Hmsc_plasticity_thin10.rda")
#load("./data/volatiles/Hmsc_plasticity_thin10.rda")
...
plotBeta(model.abu, post=postBeta, param="Support", supportLevel = 0.95, spNamesNumbers = c(F,F), covNamesNumbers = c(T,F))

postGamma = getPostEstimate(model.abu, parName="Gamma")
```
...
treatment_boxplots <- function(volsmeta.vol, treatment) {
  ggplot(volsmeta.vol, aes_string(x = "year.cluster", fill = treatment, y = "value")) + 
    ...
    labs(x="Year and cluster", y ="Emissions (ng/hr)", fill=c(water="Precipitation",snow="Snowmelt")[[treatment]]) 
}
...
ggplot(volsmeta, aes(x=year.cluster, fill=factor(paste(water, snow),levels=names(water_snow_pal)), y=total))  + 
  geom_boxplot(outlier.size=1) + 
  scale_fill_manual(values=water_snow_pal) + theme_minimal() +
  labs(x="Year and cluster", y ="Total filtered emissions (ng/hr)", fill="Precipiation, snowmelt")
```
...
ggplot(volsmeta.good.top, aes(x = VWC, color = year.cluster, y = value)) +  facet_wrap(vars(name), scales="free_y") + 
...
ggplot(volsmeta.drydown, aes(x = VWC, color = year.cluster, y = value)) +  facet_wrap(vars(name), scales="free_y") + 
  geom_point() + geom_smooth(se=F, span=1)+ theme_minimal()+ xlim(c(0,15))+ #TODO cuts off a few high VWC values
  scale_y_sqrt() + labs(x="Soil moisture (%VWC)", y ="Emissions (ng/hr)", color="Year")

ggplot(volsmeta.class, aes(x = VWC, color = year.cluster, y = value)) +  facet_wrap(vars(name), scales="free_y") + 
  geom_point() + geom_smooth(se=F, span=1)+ theme_minimal()+ xlim(c(0,15))+ #TODO cuts off a few high VWC values
  scale_y_sqrt() + labs(x="Soil moisture (%VWC)", y ="Emissions (ng/hr)", color="Year")

ggplot(volsmeta, aes(x = VWC, color = year.cluster, y = total)) + 
  geom_point() + geom_smooth(se=F, span=1)+ theme_minimal()+ xlim(c(0,15))+ #TODO cuts off a few high VWC values
...
ggplot(traits.good.top, aes(x=value, y=seeds_est, color=year.cluster)) + 
  geom_point() + geom_smooth(method="lm", se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Seed production", color="Year")+
  theme_bw()+ scale_x_sqrt()

ggplot(traits.good.top, aes(x=value, y=seeds_est, color=water, linetype=year.cluster)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
...
ggplot(traits.good.top, aes(x=value, y=seeds_est, color=snow, linetype=year.cluster)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Seed production", color="Snowmelt", linetype="Year")+
  theme_bw() + scale_color_manual(values=snow_pal)+ scale_x_sqrt()

ggplot(traits.class, aes(x=value, y=seeds_est, color=water, linetype=year.cluster)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Seed production", color="Precipitation", linetype="Year")+
  theme_bw() + scale_color_manual(values=water_pal)+ scale_x_sqrt()

ggplot(traits.class, aes(x=value, y=seeds_est, color=snow, linetype=year.cluster)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Seed production", color="Snowmelt", linetype="Year")+
  theme_bw() + scale_color_manual(values=snow_pal)+ scale_x_sqrt()

ggplot(traits.class, aes(x=total, y=seeds_est, color=water, linetype=year.cluster)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  labs(x="Total emissions", y="Seed production", color="Precipitation", linetype="Year")+
  theme_bw() + scale_color_manual(values=water_pal)+ scale_x_sqrt()

ggplot(traits.class, aes(x=total, y=seeds_est, color=snow, linetype=year.cluster)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  labs(x="Total emissions", y="Seed production", color="Snowmelt", linetype="Year")+
  theme_bw() + scale_color_manual(values=snow_pal)+ scale_x_sqrt()
```
...
ggplot(traits.good.top %>% filter(fruits_nonaborted>4), aes(x=value, y=prop_infested, color=year.cluster)) + 
  geom_point() + geom_smooth(method="lm", se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y=paste(traitnames.units["prop_infested"], "(>4 fruits)"), color="Year")+
  theme_bw()+ scale_x_sqrt()

ggplot(traits.good.top, aes(x=value, y=eggs_per_open, color=year.cluster)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Mean fly eggs / mean open flowers", color="Year")+
  theme_bw()+ scale_x_sqrt() + ylim(0,1.2)

ggplot(traits.class %>% filter(fruits_nonaborted>4), aes(x=value, y=prop_infested, color=year.cluster)) + 
  geom_point() + geom_smooth(method="lm",se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y=paste(traitnames.units["prop_infested"], "(>4 fruits)"), color="Year")+
  theme_bw()+  scale_x_sqrt()

ggplot(traits.class, aes(x=value, y=eggs_per_open, color=year.cluster)) + 
  geom_point() + geom_smooth(method="loess",span=0.8,se=F) + 
  facet_wrap(vars(name), scales="free_x") + 
  labs(x="Emissions", y="Mean fly eggs / mean open flowers")+
  theme_bw()+ scale_x_sqrt() + ylim(0,1.2)

ggplot(traits.class %>% filter(fruits_nonaborted>4), aes(x=total, y=prop_infested, color=year.cluster)) + 
  geom_point() + geom_smooth(method="loess",span=0.8,se=F) + 
  labs(x="Total emissions", y=paste(traitnames.units["prop_infested"], "(>4 fruits)"), color="Year")+
  theme_bw()+  scale_x_sqrt()

ggplot(traits.class, aes(x=total, y=eggs_per_open, color=year.cluster)) + 
  geom_point() + geom_smooth(method="loess",span=0.8,se=F) + 
  labs(x="Emissions", y="Mean fly eggs / mean open flowers", color="Year")+
  theme_bw()+  scale_x_sqrt() + ylim(0,1.2)
```
...
plot_beta(filter(vols.beta.water, year=="2018"), colorby="water") + 
  scale_color_manual(values=water_pal) + ggtitle("2018")

plot_beta(filter(vols.beta.water), colorby="water") + 
  scale_color_manual(values=water_pal) + facet_wrap(vars(year), nrow=1)
```
...
plasticity %>% left_join(vols.beta.water %>% rename(name=trait) %>% filter(water=="Addition")) %>% 
  ...
  scale_x_continuous(labels=scales::percent, limits=c(NA,2)) + theme_bw()  + theme(legend.position = "top") +
...
plasticity %>% left_join(vols.beta.water %>% rename(name=trait) %>% filter(water=="Reduction")) %>% 
  ...
  geom_vline(xintercept=0)+geom_hline(yintercept=0) + geom_text(aes(label=name)) + 
  scale_x_continuous(labels=scales::percent, limits=c(NA,4)) + theme_bw()  + theme(legend.position = "top") +
...
```{r flrsize}
ggplot(traits.class, aes(x=corolla_length, y=value, color=year.cluster)) + 
...
ggplot(traits.class, aes(x=sepal_width, y=value, color=year.cluster)) + 
  geom_point() + geom_smooth(se=F, span=1) + 
  facet_wrap(vars(name), scales="free_y") + 
  labs(y="Emissions", x=traitnames.units["sepal_width"], color="Year")+
  theme_bw() + scale_y_sqrt()

ggplot(traits.class, aes(x=nectar_24_h_ul, y=value, color=year.cluster)) + 
  geom_point() + geom_smooth(se=F, span=1) + 
  facet_wrap(vars(name), scales="free_y") + 
