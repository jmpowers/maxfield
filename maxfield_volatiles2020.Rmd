...
vols.good %>% summarize(across(everything(), mean)) %>% 
  t() %>% as.data.frame() %>% rownames_to_column("name") %>% rename(mean_emissions = V1) %>% 
  arrange(desc(mean_emissions)) %>% kable(caption = "Mean emission rates of each volatile")
```
...
plot_beta(bind_rows(vols.beta.yr, mnps.beta.yr), colorby="year")
plot_beta(bind_rows(vols.beta.water, mnps.beta.water), colorby="water")
plot_beta(bind_rows(vols.beta.snow, mnps.beta.snow), colorby="snow")
plot_beta(bind_rows(vols.beta.water.snow, mnps.beta.water.snow), colorby="water_snow") + coord_cartesian(xlim=c(-2,3.5))
```
...
adaptive.plasticity.data <- 
  ...
  left_join(bind_rows(rename(bind_rows(vols.beta.water, mnps.beta.water), newenv=water),
                    rename(bind_rows(vols.beta.snow, mnps.beta.snow), newenv=snow)) %>% 
...
adaptive.plasticity.data %>% 
  ...
  kable(caption=paste("Proportion of", ncol(vols.good.top), "volatiles and", length(floraltraits.pca), "other traits whose plasticity is adaptive across 3 yr"), digits=2)
```
