...
maxf.data <- maxf.data %>% filter(!batch %in% c("ipo2015.2016.txt"))#, #old files
                                  #"heather_maxfield190813_new.txt")) #TODO filter these dupes out later - subset of heather_maxfield190813_new001.txt
...
# Duplicate filenames in Markes/GC sequence: 
#   2x2 unique chromatograms for both Blank1_070621_1.qgd, Corydalis_200727_12_redo_762021_08.qgd (OK)
#   8x2 mismatch fuzzy matchings with one file, two close desorb times (will remove with verdict=mismatch)
maxfgc %>% drop_na(FileName) %>% group_by(FileName) %>% filter(n()>1) %>% arrange(FileName)
...
write_csv(maxfgc, "data/volatiles/maxfield_all210827_updated.csv")#used to output to the rmbl/Maxfield folder. Original from project folder backed up in RMBL 2020
