...
cat(100*years_flowering[["2"]]/(years_flowering[["1"]]+years_flowering[["2"]]), "% of plants that flowered flowered twice")
```
...
```{r soil_emms}
sm.mod %>% emmeans(~ snow)  %>% summary() %>% select(-c(df,lower.CL,upper.CL)) %>% kable()
sm.mod %>% emmeans(~ water) %>% summary() %>% select(-c(df,lower.CL,upper.CL)) %>% kable()
```
...
cat("Snowmelt is advancing",-10*tidy(meltdate.mod)$estimate[2],"+-",10*tidy(meltdate.mod)$std.error[2], "days per decade from",paste(range(groundcover$year), collapse=" - "))
